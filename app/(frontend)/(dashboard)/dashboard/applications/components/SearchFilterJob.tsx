...
const SearchFilterJob = ({ searchTerm, handleSearchJob, statusFilter, handleStatusFilter }: { searchTerm: string, handleSearchJob: (e: React.ChangeEvent<HTMLInputElement>) => void, statusFilter: string, handleStatusFilter: (e: React.ChangeEvent<HTMLSelectElement>) => void }) => {
    return (
        ...
        <div className="flex-1 relative">
            ...
            <input type="text" value={searchTerm} id="searchInput" onChange={handleSearchJob} placeholder="Search company or role..." className="w-full bg-muted rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary-hover/20 transition-all" />
        </div>
